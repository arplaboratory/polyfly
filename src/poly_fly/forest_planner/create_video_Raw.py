...
def interactive_multi_inspect(
    ...
    if policy_nn:
        ...
        policy_model = load_model_from_checkpoint(
            "/home/mrunal/Documents/poly_fly/data/zarr/models/circular_forests--forests/larger_batchsize_policy_ep0080.pth",
    ...
    if policy_model is not None:
        ...
        stats = {
            ...
            "fut_quat": DK.FUTURE_QUATERNION_DELTAS,
    ...
    state = {
        ...
        "goal_scatter": None,  # red goal dot
    }
    ...
    def ensure_depth_window(ds):
        ...
        vdata = ds.get(DK.ROBOT_VEL_WORLD, None)
        pvdata = ds.get(DK.PAYLOAD_VEL_WORLD, None)
        logits = ds.get("logits_nn", None)
        # use per-step selected mode if available; otherwise default to mode 0
        selected_mode = np.argmax(logits, axis=1) if logits is not None else None
        ...
        for j in range(4):
            ...
            # UPDATED predicted overlays: use selected_mode[k] at each stage k
            if policy_model is not None and not policy_model.use_rotation_mat:
                ...
                for k in range(0, N, plot_step_size):
                    ...
                    # horizon slice 0:seg_len at time k, mode chosen by selected_mode[k]
                    pred_seg = pquat_arr[k, mode_idx, 0:seg_len, j]
                ...
                if policy_model is not None:
                    ...
                    if pvel is not None:
                        ...
                        for k in range(0, N, plot_step_size):
                            seg_len = min(plot_step_size, N - k)
                            mode_idx = int(selected_mode[k]) if selected_mode is not None else 0
                            pred_seg = pvel_arr[k, mode_idx, 0:seg_len, j]
                            t_idx = np.arange(k + 1, k + 1 + seg_len)
                ...
                if policy_model is not None:
                    ...
                    if ppvel is not None:
                        ...
                        for k in range(0, N, plot_step_size):
                            seg_len = min(plot_step_size, N - k)
                            mode_idx = int(selected_mode[k]) if selected_mode is not None else 0
                            pred_seg = ppvel_arr[k, mode_idx, 0:seg_len, j]
                            t_idx = np.arange(k, k + seg_len)
    ...
    def clear_artists():
        ...
        for k in ["future_scatter", "future_payload_scatter", "goal_scatter"]:
            sc = state.get(k, None)
            if sc is not None:
                try:
                    sc.remove()
    ...
    def get_predicted_nn(ds):
        ...
        for i in range(robot_pos.shape[0]):
            ...
            logits_nn[i, :] = logits.cpu().numpy().astype(np.float32)  # (n_modes,)
            ...
            # R_yaw = R.from_euler('z', yaw, degrees=False)  # (x,y,z,w)
            # R_yaw = R.from_quat(robot_quat[i, :]).as_matrix()

            for k in range(predicted_payload_pos_nn.shape[1]):
                for h in range(predicted_payload_pos_nn.shape[2]):
                    # predicted_robot_pos_nn[i, k, h, :] = R.from_quat(robot_quat[i, :]).apply(un_pos[k, h, :])
                    # predicted_robot_pos_nn[i, k, h, :] = R_yaw.apply(un_pos[k, h, :])
                    predicted_robot_pos_nn[i, k, h, :] = un_pos[k, h, :]
            ...
            if use_rotation_mat:
                for j in range(n_modes):
                    for k in range(horizon):
                        predicted_rot_mat_nn[i, j, k, :] = un_rot_mat[j, k, :] + robot_rot_mat[i, :]
                        # rot_mat = process_rotation_matrix(np.expand_dims(predicted_rot_mat_nn[i, j, k, :], axis=0)).squeeze(0)
                        # predicted_rot_mat_nn[i, j, k, :] = rot_mat[:, :2].flatten()
                        # predicted_robot_quat_nn[i, j, k, :] = R.from_matrix(rot_mat).as_quat()
            else:
                predicted_robot_quat_nn[i, :, :, :] = un_quat + robot_quat[i, :]
    ...
    def apply_dataset(k):
        ...
        payload = ds.get(DK.PAYLOAD_POS, None)
        payload_world = ds.get(DK.PAYLOAD_POS_WORLD, None)
        ...
        goal_world_pos = goal_world + quad
        goal_world_pos = ds[DK.ROBOT_GOAL]
        print("goal world pos:", goal_world_pos)
        ######
        ...
        def draw_at(i):
            ...
            # ------------------------------------------------------------------
            # Draw 2D yaw vector at robot center (in X-Y plane)
            # yaw is extracted from quaternion quat[i, :]
            # 2D vector = (cos(yaw), sin(yaw))
            # ------------------------------------------------------------------
            # compute yaw from quaternion (x, y, z, w)
            r_yaw = R.from_quat(q)
            yaw = r_yaw.as_euler("zyx", degrees=False)[0]  # [yaw, pitch, roll]
            dir_xy = np.array([np.cos(yaw), np.sin(yaw)])

            # start at robot position in XY, end one unit along yaw direction
            start_xy = pQ[0:2]
            end_xy = start_xy + 0.5 * dir_xy  # you can scale dir_xy if you want a longer arrow

            # remove previous yaw arrow if present
            yaw_artist = state.get("yaw_arrow", None)
            if yaw_artist is not None:
                try:
                    yaw_artist.remove()
                except ValueError:
                    pass
            ...
            if state.get("goal_scatter", None) is not None:
                g = goal_world_pos
                state["goal_scatter"]._offsets3d = ([g[0]], [g[1]], [g[2]])
...
def main():
    ...
    if args.zarr:
        ...
        if args.inspect and args.combined:
            datasets = load_zarr_folder(zpath, args, limit=2, random_sample=True)
